...
const UploadProducts = () => {
 ...
 useEffect(() => {
    const stored = JSON.parse(localStorage.getItem('uploadedProducts')) || [];
    setProducts(stored);
    setFiltered(stored);
  }, []);
...
const handleFinalUpload = () => {
  ...
  localStorage.setItem('uploadedProducts', JSON.stringify(updatedList));
  setProducts(updatedList);
  setEditIndex(null);
  setFormData(initialForm);
  setVariants([{ color: '#800000', size: 'S', quantity: 1 }]);
  setPreviewCard(false);
  setSelectedImage(null); // clear after use
};
...
/>
            ...
            >
              <FaPlus style={{ marginRight: '6px' }} /> New Product
            </button>
          </div>
        </div>
                      ...
                      <tr key={i} style={{ textAlign: 'center', color: '#000' }}>
                        ...
                        <td style={cell}>
                          <div style={{ display: 'flex', justifyContent: 'center', gap: '10px' }}>
                            ...
                            <button onClick={() => handleDelete(i)} style={{ ...iconBtn, color: 'red' }} title="Delete"><FaTrash /></button>
                          </div>
                        </td>
                      </tr>
                    );
